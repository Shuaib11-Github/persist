...
with tab1:
    ...
    if company == "Overview":
        ...
        # Summary for individual company
        st.markdown(f"""
        ### Summary for **{company}**:
...
with tab2:
    ...
    # Summary based on dynamic aggregated data
    st.markdown(f"""
    ### Summary for **{company}**:
...
with tab5:
    ...
    if not countries_df.empty:
        # Get a DataFrame of all countries with their ISO codes
        df_countries = px.data.gapminder()[['country', 'iso_alpha']].drop_duplicates()

        # Merge your data with the countries DataFrame to get ISO codes
        ...
        # Summary based on aggregated data
        st.markdown(f"""
        ### Summary for **{company}**:
...
#     # Get a DataFrame of all countries with their ISO codes (Plotly provides this in gapminder)
#     df_countries = px.data.gapminder()[['country', 'iso_alpha']].drop_duplicates()

#     # Merge the country data with your own data to include all countries, even those without data
#     countries_df = df_countries.merge(countries_df, left_on='country', right_on='Country', how='left')

#     # Add 0 as a default value for 'Global South Deals Funded' if there is no data
#     countries_df['Global South Deals Funded'] = countries_df['Global South Deals Funded'].fillna(0)

#     # Create the map
#     fig = go.Figure()

#     # Add markers for countries with data, adjust size and color
#     fig.add_trace(go.Scattergeo(
#         locations=countries_df['iso_alpha'],
#         locationmode='ISO-3',
#         text=countries_df['country'],  # Display country name when hovering
#         mode='markers',
#         marker=dict(
#             symbol='circle',
#             size=countries_df['Global South Deals Funded'] * 2,  # Increase size for visibility
#             sizemode='area',
#             sizeref=2.*max(countries_df['Global South Deals Funded'])/(40.**2),
#             sizemin=4,
#             color=countries_df['Global South Deals Funded'],  # Color based on deals funded
#             colorscale='Viridis',  # Heatmap-like color scale
#             colorbar=dict(title="Deals Funded"),  # Show a colorbar on the side
...
#         ),
#         hoverinfo='text',  # Only show the country name as hover text
#         showlegend=False
#     ))

#     # Add a layout to display country boundaries, show labels and make the map wider and taller
#     fig.update_geos(
#         showland=True,
#         landcolor='black',
...
#         showframe=False,
#         coastlinecolor='white',
#         showsubunits=True,  # Show subunits like states/regions
#         subunitcolor='white',
#         projection_type='equirectangular',
#         resolution=50,
#     )

#     # Adjust the layout size and background color
#     fig.update_layout(
#         title='Global South Deals Funded by Country',
#         geo=dict(
#             showland=True,
...
#     # Only calculate the summary based on actual data (ignoring countries with 0 deals)
#     countries_with_data = countries_df[countries_df['Global South Deals Funded'] > 0]

#     if not countries_with_data.empty:
#         # Dynamically find the country with the highest Global South Deals Funded
#         highest_country = countries_with_data.loc[countries_with_data['Global South Deals Funded'].idxmax()]['country']
#         lowest_country = countries_with_data.loc[countries_with_data['Global South Deals Funded'].idxmin()]['country']

#         highest_deals = countries_with_data['Global South Deals Funded'].max()
#         lowest_deals = countries_with_data['Global South Deals Funded'].min()
#         total_deals = countries_with_data['Global South Deals Funded'].sum()

#         # Summary based on aggregated data
#         st.markdown(f"""
#         ### Summary for **{company}**:
        
#         - **Country with the Highest Global South Deals Funded**: **{highest_country}** with **{highest_deals} deals**
#         - **Country with the Smallest Global South Deals Funded**: **{lowest_country}** with **{lowest_deals} deals**
#         - **Total Global South Deals Funded**: **{total_deals} deals**
#         """)
#     else:
#         st.write("No Global South Investments data available for the selected company.")
...
#             locations=countries_df['iso_alpha'],
#             locationmode='ISO-3',
#             text=countries_df.apply(lambda row: f"Country: {row['Country']}<br>Deals Funded: {row['Global South Deals Funded']}", axis=1),
#             mode='markers',
#             marker=dict(
#                 symbol='circle',
#                 size=countries_df['Global South Deals Funded'] * 2,
#                 sizemode='area',
#                 sizeref=2.*max(countries_df['Global South Deals Funded'])/(40.**2),
#                 sizemin=4,
#                 color=countries_df['Global South Deals Funded'],
#                 colorscale='Viridis',
#                 colorbar=dict(title="Deals Funded"),
#                 line=dict(width=1, color='black')
#             ),
#             hoverinfo='text',
#             showlegend=False
